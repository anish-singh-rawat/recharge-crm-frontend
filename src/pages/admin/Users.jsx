...
import { z } from 'zod'
import { UserPlus, Search, Ban, CheckCircle, Trash2, Eye } from 'lucide-react'
    ...
    <Modal open={open} onClose={onClose} title="Create User" size="md">
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Input label="Business Name" placeholder="Sharma Telecom" {...register('businessName')} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Input label="Email" type="email" placeholder="rahul@example.com" error={errors.email?.message} required {...register('email')} />
          <Input label="Phone" placeholder="9876543210" error={errors.phone?.message} required {...register('phone')} />
        </div>
        <Select label="Role" options={roles} placeholder="Select role" error={errors.role?.message} required {...register('role')} />
        <div className="grid grid-cols-2 gap-3">
          <Input label="Password" type="password" placeholder="Min 8 chars" error={errors.password?.message} required {...register('password')} />
          <Input label="Confirm Password" type="password" placeholder="Re-enter" error={errors.confirmPassword?.message} required {...register('confirmPassword')} />
        </div>
        <div className="flex gap-3 pt-2">
          <Button variant="secondary" className="flex-1" onClick={onClose} type="button">Cancel</Button>
          <Button className="flex-1" type="submit" loading={mutation.isPending}>Create User</Button>
        </div>
      </form>
    </Modal>
  )
...
export default function Users() {
  ...
  return (
    <div className="space-y-6">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]">
                    {['User', 'Phone', 'Role', 'Business', 'Status', 'Created', 'Actions'].map((h) => (
                ...
                <tbody>
                  {data.items.map((user) => (
                    <tr key={user._id} className="border-b border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <Badge variant={user.role === 'super_admin' ? 'purple' : user.role === 'admin' ? 'primary' : 'default'}>
                          {user.role?.replace('_', ' ')}
                        </Badge>
                      </td>
      ...
      <Modal open={!!blockModal} onClose={() => setBlockModal(null)} title="Block User" size="sm">
        <div className="space-y-3">
          <p className="text-sm text-[#475569]">Block <strong>{blockModal?.name}</strong>?</p>
          <Input label="Reason" placeholder="Suspicious activity" value={blockReason} onChange={(e) => setBlockReason(e.target.value)} />
          <div className="flex gap-3 pt-2">
            <Button variant="secondary" className="flex-1" onClick={() => setBlockModal(null)}>Cancel</Button>
            <Button variant="danger" className="flex-1" onClick={() => blockMutation.mutate({ id: blockModal._id, reason: blockReason })} loading={blockMutation.isPending}>
              Block
            </Button>
          </div>
        </div>
        ...
        {viewUser && (
          <div className="space-y-3 text-sm">
            {[
              ...
              <div key={label} className="flex justify-between py-1.5 border-b border-[#E2E8F0] last:border-0">
